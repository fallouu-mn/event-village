...
export async function GET(req: NextRequest) {
    try {
        ...
        const format = (searchParams.get('format') === 'xlsx' ? 'xlsx' : 'csv') as 'xlsx' | 'csv';

        const { buffer, contentType, filename } = await PartnerDashboardService.generateAccountingExport(user.id, eventId, format);
        ...
        console.error('[GET /api/partner/reports/accounting] Erreur:', err);
